...
namespace WavTexture
{
    ...
    class LevelMonitor : MonoBehaviour
    {
        ...
        public Material ShaderA, ShaderB, ShaderC;
        public string ShaderAFl, ShaderBFl, ShaderCFl;
        void Update()
        {
            ...
            // громкость звука в шейдер
            var texture = new Texture2D(_resultBuffer.width, _resultBuffer.height, _resultBuffer.graphicsFormat, 0);
            RenderTexture.active = _resultBuffer;
            texture.ReadPixels(new Rect(0, 0, _resultBuffer.width, _resultBuffer.height), 0, 0);
            texture.Apply();
            ShaderA.SetFloat(ShaderAFl, texture.GetPixel(0,0).r);
            ShaderB.SetFloat(ShaderAFl, texture.GetPixel(0, 0).r);
            ShaderC.SetFloat(ShaderAFl, texture.GetPixel(0, 0).r);
            //Debug.Log("_BitA"+texture.GetPixel(0, 0).r);
        }
